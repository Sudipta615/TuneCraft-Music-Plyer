...
impl GainRamp {
    fn immediate(gain: f32) -> Self {
        Self { gain, target: gain, step: 0.0 }
    }
    ...
    #[allow(dead_code)]
    fn is_done(&self) -> bool { self.step == 0.0 }
}
...
impl CrossfadeEntry {
    fn new(uri: &str, pipeline: &Pipeline, mixer: &Element) -> Result<Self> {
        ...
        uridecodebin.connect_pad_added(move |_, pad| {
            if let (Some(pipeline), Some(mixer)) =
                (pipeline_weak.upgrade(), mixer_weak.upgrade())
            {
    ...
    fn install_gain_probe(&self) {
        let Some(src_pad) = self.volume.static_pad("src") else { return };
...
impl CrossfadeEngine {
    pub fn new(fade_duration_ms: u32) -> Result<Self> {
        ...
        let mixer        = ElementFactory::make("audiomixer").build().context("audiomixer")?;
        let audioconvert = ElementFactory::make("audioconvert").build().context("audioconvert")?;
        let audiosink    = ElementFactory::make("autoaudiosink").build().context("autoaudiosink")?;
            ...
            while let Some(msg) = bus.pop() {
                match msg.view() {
                    ...
                    gstreamer::MessageView::Error(e) =>
                        tracing::error!("CrossfadeEngine GST error: {} ({:?})", e.error(), e.debug()),
    ...
    pub fn load_track(&self, uri: &str) -> Result<()> {
        ...
        let fade_ms = *self.fade_duration_ms.lock().unwrap_or_else(|e| e.into_inner());
        let vol     = *self.current_volume.lock().unwrap_or_else(|e| e.into_inner());
    ...
    pub fn play(&self) { let _ = self.pipeline.set_state(gstreamer::State::Playing); }
    pub fn pause(&self) { let _ = self.pipeline.set_state(gstreamer::State::Paused); }

    pub fn toggle_playback(&self) {
        let state = self.pipeline.current_state();
        if state == gstreamer::State::Playing { self.pause() } else { self.play() }
    }
    ...
    pub fn duration(&self) -> Option<Duration> {
        ...
        let r = self.pipeline.query_duration::<gstreamer::ClockTime>()
            .map(|ct| Duration::from_nanos(ct.nseconds()));
        if r.is_some() {
            if let Ok(mut d) = self.track_duration.lock() { *d = r; }
        }
        r
    }

    pub fn set_duration(&self, dur: Option<Duration>) {
        if let Ok(mut d) = self.track_duration.lock() { *d = dur; }
    }

    pub fn fade_duration(&self) -> Duration {
        Duration::from_millis(*self.fade_duration_ms.lock().unwrap_or_else(|e| e.into_inner()) as u64)
    }

    pub fn set_fade_duration(&self, ms: u32) {
        *self.fade_duration_ms.lock().unwrap_or_else(|e| e.into_inner()) = ms;
    }

    pub fn set_volume(&self, vol: f64) {
        *self.current_volume.lock().unwrap_or_else(|e| e.into_inner()) = vol.clamp(0.0, 1.0);
        for e in self.entries.lock().unwrap_or_else(|e| e.into_inner()).iter() {
    ...
    pub fn set_rate(&self, rate: f64) {
        ...
        let pos = match self.pipeline.query_position::<gstreamer::ClockTime>() {
            Some(p) => p,
            None => {
                tracing::warn!("CrossfadeEngine set_rate: query_position returned None, skipping rate change");
                return;
            }
        };
