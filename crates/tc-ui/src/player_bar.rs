...
fn truncate(ui: &Ui, text: &str, font: &FontId, max_width: f32) -> String {
    let g = ui.painter().layout_no_wrap(text.to_string(), font.clone(), Color32::WHITE);
    if g.size().x <= max_width { return text.to_string(); }
    let eg = ui.painter().layout_no_wrap("...".to_string(), font.clone(), Color32::WHITE);
    let target = max_width - eg.size().x;
    if target <= 0.0 { return "...".to_string(); }
    ...
    while lo < hi {
        ...
        let pg = ui.painter().layout_no_wrap(text[..end].to_string(), font.clone(), Color32::WHITE);
        if pg.size().x <= target { lo = mid; } else { hi = mid - 1; }
    }
...
fn draw_full(app: &mut TuneCraftApp, ui: &mut Ui, colors: &TuneCraftColors, bar_rect: Rect, total_w: f32) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.painter().rect_stroke(art_r, 4.0, egui::Stroke::new(1.0, colors.border));
        ui.painter().text(art_r.center(), Align2::CENTER_CENTER, "\u{266A}", FontId::proportional(20.0), colors.text_dim);

        ui.add_space(8.0);

        let track_info_w = (info_w - art_size - 40.0).max(60.0);
        let (info_rect, _) = ui.allocate_exact_size(Vec2::new(track_info_w, PLAYER_BAR_HEIGHT), Sense::hover());
        ...
        if let Some(track) = app.current_track() {
            ...
            ui.painter().text(Pos2::new(info_rect.left(), cy - 10.0), Align2::LEFT_CENTER, &title, title_font, colors.text);

            let artist_font = FontId::proportional(12.0);
            let artist = track.artist.as_deref().unwrap_or("Unknown Artist");
            let artist_t = truncate(ui, artist, &artist_font, max_title_w);
            ui.painter().text(Pos2::new(info_rect.left(), cy + 10.0), Align2::LEFT_CENTER, &artist_t, artist_font, colors.text_dim);
        } else {
            ui.painter().text(Pos2::new(info_rect.left(), cy), Align2::LEFT_CENTER, "No track playing", FontId::proportional(14.0), colors.text_dim);
        }

        // Heart button
        let heart_rect = Rect::from_center_size(Pos2::new(info_rect.right() + 12.0, cy), Vec2::new(24.0, 24.0));
        let heart_resp = ui.interact(heart_rect, egui::Id::new("fav_heart"), Sense::click());
        let heart_color = if app.is_favorited { Color32::from_rgb(0xEF, 0x44, 0x44) } else { colors.text_dim };
        ui.painter().text(heart_rect.center(), Align2::CENTER_CENTER, "\u{2665}", FontId::proportional(20.0), heart_color);
        if heart_resp.clicked() { app.toggle_favorite(); }
            ...
            ui.vertical(|ui| {
                ...
                ui.horizontal(|ui| {
                    ui.with_layout(egui::Layout::centered_and_justified(egui::Direction::LeftToRight), |ui| {
                        let shuffle_color = if app.shuffle { colors.accent } else { colors.text_dim };
                        ...
                        let play_label = if app.is_playing { "\u{23F8}" } else { "\u{25B6}" };
                        let (pb_rect, pb_resp) = ui.allocate_exact_size(Vec2::new(play_btn_size, play_btn_size), Sense::click());
                        let pb_bg = if pb_resp.hovered() { colors.accent_dark } else { colors.accent };
                        ui.painter().circle_filled(pb_rect.center(), play_radius, pb_bg);
                        ui.painter().text(pb_rect.center(), Align2::CENTER_CENTER, play_label, FontId::proportional(play_btn_size * 0.42), Color32::WHITE);
                        if pb_resp.clicked() { app.toggle_playback(); }
                ...
                ui.add_space(2.0);
                ui.horizontal(|ui| {
                    let track_duration = if app.duration_secs > 0.0 { app.duration_secs } else { 1.0 };
                    ...
                    ui.label(RichText::new(format!("{}:{:02}", pos_m, pos_s)).font(FontId::proportional(10.0)).color(colors.text_dim));

                    // Progress bar uses remaining width
                    let prog_w = ui.available_width() - 36.0;
                    let prog_h = 14.0;
                    let (prog_rect, prog_resp) = ui.allocate_exact_size(Vec2::new(prog_w, prog_h), Sense::click());
                    let py = prog_rect.center().y;

                    ui.painter().line_segment([Pos2::new(prog_rect.left(), py), Pos2::new(prog_rect.right(), py)], egui::Stroke::new(4.0, colors.slider_track));
                    let fill_x = prog_rect.left() + prog_rect.width() * progress;
                    ui.painter().line_segment([Pos2::new(prog_rect.left(), py), Pos2::new(fill_x, py)], egui::Stroke::new(4.0, colors.slider_fill));
                    ui.painter().circle_filled(Pos2::new(fill_x, py), 6.0, colors.accent);
                    ...
                    ui.label(RichText::new(format!("{}:{:02}", dur_m, dur_s)).font(FontId::proportional(10.0)).color(colors.text_dim));
                });
            });
        });
        ...
        ui.vertical(|ui| {
            ui.add_space(14.0);
            ui.horizontal(|ui| {
                let vol_icon = if app.volume < 0.01 { "\u{1F507}" } else if app.volume < 0.5 { "\u{1F508}" } else { "\u{1F509}" };
                ui.label(RichText::new(vol_icon).font(FontId::proportional(14.0)).color(colors.text_dim));

                // Custom-drawn volume slider — width adapts to available space
                let vol_w = ui.available_width().min(120.0).max(40.0);
                let vol_h = 14.0;
                let (vol_rect, vol_resp) = ui.allocate_exact_size(Vec2::new(vol_w, vol_h), Sense::click_and_drag());
                let vy = vol_rect.center().y;
                let vol = app.volume as f32;

                ui.painter().line_segment(
                    [Pos2::new(vol_rect.left(), vy), Pos2::new(vol_rect.right(), vy)],
                ...
                ui.painter().circle_filled(Pos2::new(fill_x, vy), 5.0, colors.accent);
            ...
            ui.horizontal(|ui| {
                let lyrics_color = if app.show_lyrics { colors.accent } else { colors.text_dim };
                ...
                if total_w > 800.0 {
                    let speed_label = if (app.speed - 1.0).abs() < 0.01 { "1x".to_string() } else { format!("{:.1}x", app.speed) };
                    let speed_color = if (app.speed - 1.0).abs() < 0.01 { colors.text_dim } else { colors.accent };
                    ui.label(RichText::new(&speed_label).font(FontId::proportional(10.0)).color(speed_color));

                    for &s in &[0.5f64, 0.75, 1.0, 1.25, 1.5, 2.0] {
                        let is_cur = (app.speed - s).abs() < 0.01;
                        let label = if s == s.round() { format!("{}x", s as u32) } else { format!("{:.1}x", s) };
                        if ui.add(egui::Button::new(
                            RichText::new(&label).font(FontId::proportional(9.0)).color(if is_cur { colors.accent } else { colors.text_dim })
                        ).frame(false)).clicked() {
                            app.set_speed(s);
                        }
                    }
                    ...
                    let speed_color = if (app.speed - 1.0).abs() < 0.01 { colors.text_dim } else { colors.accent };
                    if ui.add(egui::Button::new(
                        RichText::new(&speed_label).font(FontId::proportional(10.0)).color(speed_color)
                    ).frame(false)).clicked() {
                        // Cycle through speeds
                        let speeds = [0.5f64, 0.75, 1.0, 1.25, 1.5, 2.0];
                        let cur_idx = speeds.iter().position(|&s| (app.speed - s).abs() < 0.01).unwrap_or(2);
...
fn draw_compact(app: &mut TuneCraftApp, ui: &mut Ui, colors: &TuneCraftColors, bar_rect: Rect, total_w: f32) {
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.painter().rect_filled(art_r, 4.0, colors.card);
            ui.painter().rect_stroke(art_r, 4.0, egui::Stroke::new(1.0, colors.border));
            ui.painter().text(art_r.center(), Align2::CENTER_CENTER, "\u{266A}", FontId::proportional(14.0), colors.text_dim);
            ...
            if let Some(track) = app.current_track() {
                ...
                ui.painter().text(Pos2::new(info_rect.left(), info_rect.center().y), Align2::LEFT_CENTER, &title, title_font, colors.text);
            } else {
                ui.painter().text(info_rect.center(), Align2::CENTER_CENTER, "No track", FontId::proportional(12.0), colors.text_dim);
            }

            // Center controls
            ui.with_layout(egui::Layout::centered_and_justified(egui::Direction::LeftToRight), |ui| {
                if icon_btn(ui, "\u{23EE}", 16.0, colors.text) { app.play_prev(); }
                let play_label = if app.is_playing { "\u{23F8}" } else { "\u{25B6}" };
                let (pb_rect, pb_resp) = ui.allocate_exact_size(Vec2::new(36.0, 36.0), Sense::click());
                let pb_bg = if pb_resp.hovered() { colors.accent_dark } else { colors.accent };
                ui.painter().circle_filled(pb_rect.center(), 18.0, pb_bg);
                ui.painter().text(pb_rect.center(), Align2::CENTER_CENTER, play_label, FontId::proportional(14.0), Color32::WHITE);
                if pb_resp.clicked() { app.toggle_playback(); }
                if icon_btn(ui, "\u{23ED}", 16.0, colors.text) { app.play_next(); }
            });
            ...
            if icon_btn(ui, vol_icon, 14.0, colors.text_dim) {
                ...
                if app.volume > 0.0 { app.set_volume(0.0); } else { app.set_volume(0.7); }
            }
        });
        ...
        ui.horizontal(|ui| {
            ...
            let pos_s = (app.position_secs % 60.0) as u32;
            let pos_m = (app.position_secs / 60.0) as u32;
            ui.label(RichText::new(format!("{}:{:02}", pos_m, pos_s)).font(FontId::proportional(9.0)).color(colors.text_dim));
            ...
            ui.painter().line_segment([Pos2::new(prog_rect.left(), py), Pos2::new(prog_rect.right(), py)], egui::Stroke::new(3.0, colors.slider_track));
            let fill_x = prog_rect.left() + prog_rect.width() * progress;
            ui.painter().line_segment([Pos2::new(prog_rect.left(), py), Pos2::new(fill_x, py)], egui::Stroke::new(3.0, colors.slider_fill));
            ui.painter().circle_filled(Pos2::new(fill_x, py), 4.0, colors.accent);
            ...
            let dur_s = (track_duration % 60.0) as u32;
            let dur_m = (track_duration / 60.0) as u32;
            ui.label(RichText::new(format!("{}:{:02}", dur_m, dur_s)).font(FontId::proportional(9.0)).color(colors.text_dim));
        });
    });
}
...
fn icon_btn(ui: &mut Ui, icon: &str, size: f32, color: Color32) -> bool {
    ui.add(
        egui::Button::new(RichText::new(icon).font(FontId::proportional(size)).color(color))
            .frame(false)
    ).clicked()
}
