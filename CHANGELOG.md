...
## [3.1.5] — 2026-06-21
### 🐛 Fixed
- Fixed lyrics panel not rendering and correctly wired the toggle button.
- Fixed sidebar badge counts showing 0 instead of actual track counts.
- Fixed track-list pagination not updating the view when changing pages.

---

## [3.1.4-patch1] — 2026-06-21
### 🐛 Fixed
- Fixed theme switching being completely broken.
- Fixed pagination off-by-one error allowing navigation past the end.
- Fixed sort-direction toggle behavior when changing columns.
- Fixed UI padding issues on toast text, dialogs, playlist rows, and sidebar headers.

### 🧹 Cleanup
- Removed dead code and unused widget files left over from the egui migration.
- Removed unnecessary `mut` bindings and redundant type casts.

---

## [3.1.4] — 2026-06-21
Migrated the entire UI from egui to Slint with femtovg GPU renderer for lower CPU usage on low-end hardware and a more modern look-and-feel.

### 🎨 UI
- Reimplemented all components (Sidebar, player bar, track list, EQ panel, etc.) in Slint.
- Preserved all 11 themes and embedded Phosphor-equivalent SVG icons.
- State-sync timer (200ms) replaces the continuous per-frame update loop.

### 🏗️ Architecture
- Retained the existing service layer and app state struct unchanged.
- Added `converters` module to translate Rust types to Slint structs.
- `app.slint` and related components added to `crates/tc-ui/ui/`.

### 🛠️ Build
- Rust 1.85+ is effectively required for the `tc-ui` workspace.

### ⚠️ Known Limitations
- Cover art currently uses a placeholder.
- In-app keyboard shortcuts and the EQ spectrum analyzer are not yet ported to Slint.

---

## [3.1.3] — 2026-06-20
### 🛠️ Fixed
- Fixed compile errors against the latest egui 0.34 patch release.
- Wired up `--help` and `--version` CLI flags to output and exit cleanly.
- Fixed obsolete scrobble regression test.

### ✨ Added
- Exposed LRCLIB endpoint configuration in the Settings UI.
- Added V011 database migration to correctly set `app_version` on fresh installs.

---

## [3.1.2] — 2026-06-20
### 🛠️ Fixed
- Stereo Enhancer and Dither now correctly restore their states after leaving LowPower mode.
- Fixed double-counting bug in `tracks.play_count` during scrobbling.
- Fixed crossfade position drifting at playback speeds other than 1.0x.
- Mute toggle now accurately restores the pre-mute volume instead of a hardcoded value.

---

## [3.1.1] — 2026-06-20
### 🛠️ Fixed
- `--analyze` CLI flag now works correctly and backfills missing BPM/loudness metadata.
- Delete-Folder button now correctly removes tracks from the DB.
- EQ sliders now respond correctly to clicks and dragging.
- Configured LRCLIB `base_url` is now respected instead of hardcoding the URL.
- Fixed in-app keyboard shortcuts.
- Handled all `MediaKeyAction` variants from external MPRIS clients.
- Play count no longer increments if a track fails to load.
- Queue now refreshes dynamically whenever a library scan completes.
- Background analysis runs automatically after adding new folders.
- Headless mode CPU usage significantly reduced.
- Fixed duplicate MPRIS D-Bus services on Linux.
- Changing `MultibandCompressor` sample rate preserves user settings.
- Removed phase inversion from the crossover filter to flatten sum response.
- Fixed sample conversion off-by-one errors for i16 and u16 outputs.
- Cleared pending audio output frames on Stop and track changes to prevent glitches.
- Adjusted crossfade trigger to be speed-aware for variable-speed playback.
- Made scrobble writes fully transactional.
- Cleaned up orphaned artists from the database when deleting tracks.

### 🚀 Added
- Added database query for tracks missing analysis (BPM, loudness, ReplayGain).
- Added bounds validation for `BandCompressorConfig` fields.

---

## [3.1.0] — 2026-06-18
### 🚀 Added
- Added a real-time Spectrum Analyzer in the EQ panel.
- Added release packaging script (`dist/build-release.sh`).
- Added Benchmark Regression and Audio-Device CI jobs.
- Extracted reusable UI widgets (HeartButton, AlbumArt, etc.).

### ✨ Changed
- Optimized the `LowPower` DSP profile to bypass convolution, multiband compressor, stereo enhancer, dither, and spectrum analyzer (reduces CPU usage by ~60%).
- Output buffer size is now adaptive based on the performance mode.
- Throttled UI repaint rates dynamically based on activity (down to 1 Hz when paused).
- Improved SIMD vectorization for Parametric EQ processing.

### 🐛 Fixed
- Added missing accessor for `MultibandCompressor::is_enabled` to preserve user preferences.

---

## [3.0.0] — 2026-06-18
### 🚀 Added
- Added EBU R128 / ITU-R BS.1770-4 integrated loudness measurement and ReplayGain 2.0 support.
- Added synced lyrics fetching via LRCLIB with database caching.
- Added CI workflows and supply-chain safety checks (`cargo-deny`).

### ✨ Changed
- Throttled MPRIS position updates to 1 Hz to drastically reduce Linux CPU usage.
- Optimized engine tick polling and CPAL device-change watcher to save CPU.
- Replaced `std::sync::Mutex` with `parking_lot::Mutex` in `tc-db` for better performance.
- Improved the Lookahead Limiter with soft-knee saturation and a 10 ms lookahead window.

### 🐛 Fixed
- Fixed smoke tests failing due to an outdated database schema reference.
- Corrected README documentation to accurately reflect implemented features.
